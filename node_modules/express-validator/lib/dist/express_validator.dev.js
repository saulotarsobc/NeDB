...
var chainCreator = require('./chain-creator');

var getCustomMethods = require('./custom-methods');

var contextRunner = require('./legacy-runner'); // validators and sanitizers not prefixed with is/to
...
var expressValidator = function expressValidator(options) {
  ...
  function Sanitizer(param, req, locations) {
    ...
    utils.mapAndExtend(getCustomMethods(req, options).sanitizers, Sanitizer.prototype, utils.makeSanitizer);
    return this;
  }
  /**
  ...
  function validateSchema(schema, req, loc, contexts) {
    checkSchema(schema, loc, function (field, locations, message) {
      return chainCreator(field, locations, message, contexts, getCustomMethods(req, options));
    });
  }
  /**
  ...
  return function (req, res, next) {
    ...
    var runContexts = function runContexts() {
      return contextRunner(contexts, req);
    };

    var locations = ['body', 'params', 'query', 'cookies']; // Extend existing validators. Fixes bug #341

    var customMethods = getCustomMethods(req, options);
