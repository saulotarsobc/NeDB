...
describe('Basic CRUD functionality', function () {
      ...
      db.insert({
        ...
        db.insert({
          ...
          db.find({
            ...
            var doc2 = _.find(docs, function (doc) {
              return doc._id === newDoc2._id;
            }),
                doc3 = _.find(docs, function (doc) {
              return doc._id === newDoc3._id;
            });
    ...
    db.insert({
      ...
      db.insert({
        ...
        db.insert({
          ...
          db.insert({
            planet: 'Eaaaaaarth'
          }, function (err, newDoc4) {
            assert.isNull(err);
            db.insert({
              planet: 'Maaaars'
            }, function (err, newDoc5) {
              assert.isNull(err);
              db.find({
                planet: /ar/
              }, function (err, docs) {
                ...
                assert.equal(_.find(docs, function (doc) {
                ...
                assert.equal(_.find(docs, function (doc) {
                  return doc._id === newDoc4._id;
                }).planet, 'Eaaaaaarth');
                assert.equal(_.find(docs, function (doc) {
                  return doc._id === newDoc5._id;
                }).planet, 'Maaaars');
                db.find({
                  planet: /aa+r/
                }, function (err, docs) {
      ...
      db.insert({
        ...
        db.update({
          ...
          db.find({}, function (err, docs) {
            ...
            assert.equal(findById(docs, newDoc2._id).planet, 'Saturn'); // Failing update
            ...
            db.update({
              ...
              db.find({}, function (err, docs) {
                assert.equal(docs.length, 2);
                assert.equal(findById(docs, newDoc1._id).planet, 'Eaaaaarth');
                assert.equal(findById(docs, newDoc2._id).planet, 'Saturn'); // Document replacement
                ...
                db.update({
                  ...
                  db.find({}, function (err, docs) {
                    ...
                    assert.equal(findById(docs, newDoc2._id).planet, 'Saturn'); // Multi update
  ...
  it('Upserts', function (done) {
    ...
    db.update({
      ...
      assert.isNull(err); // Return upserted document
  ...
  it('Removing documents', function (done) {
    ...
    db.remove({
      ...
      db.find({}, function (err, docs) {
        assert.equal(docs.length, 1);
        assert.equal(docs[0].a, 2); // Remove with no match
        ...
        db.remove({
          ...
          db.find({}, function (err, docs) {
            assert.equal(docs.length, 1);
            assert.equal(docs[0].a, 2); // Simple remove
    ...
    db.insert({
      ...
      db.insert({
        ...
        db.insert({
          ...
          var candidates = db.getCandidates({
            a: 6
          });
          assert.equal(candidates.length, 3);
          assert.isDefined(_.find(candidates, function (doc) {
            return doc.a === 4;
          }));
          assert.isDefined(_.find(candidates, function (doc) {
            return doc.a === 6;
